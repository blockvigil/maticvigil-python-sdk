...
'''
Messages raised as {"contract": contract_address, "missing": [1|2]}
1: If no entry for the contract address exists on Redis
2: Entry for the contract address exists, but the ABI file is not loaded and by inference, the mappings would be invalid or absent.
#2 is mostly the reason of some cache invalidation and race conditions that might come up in the future
'''
